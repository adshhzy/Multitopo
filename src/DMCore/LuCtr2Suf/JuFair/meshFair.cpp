...
void LuMesh::InputData( const std::vector<float> &mver,const intvector &mface, const intvector &mfaceMat,
    const intvector &ctrmedge, const bool &doswap){

    InputData( mver,mface, ctrmedge,doswap);

    for(int i=0;i<suffacenum*2;++i)sufmat[i] = mfaceMat[i];
...
void LuMesh::OutputData( std::vector<float> &mver, intvector &mface,intvector &mfaceMat){

    OutputData(mver,mface);
    int ind2 = suffacenum*2;
    mfaceMat.resize(ind2);
